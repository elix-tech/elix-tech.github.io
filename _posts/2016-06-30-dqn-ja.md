...
<br>
AMIは`DQN-AMI`という名前で、AMI IDは`ami-c4a969a9`です。リージョンはN. Virginiaです。Ubuntu上にTensorFlow, Keras, scikit-image, OpenAI Gym, CUDA, cuDNNがプリインストールしてあります。コードは`scp`で転送するか、インスタンス上で`git clone`をして取ってきてください。  
<br>
